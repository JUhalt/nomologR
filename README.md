...
`nomologR` is distributed through R-universe, which builds the latest
GitHub release. CRAN submission is planned for a later polish release
...
`nomo_report()` creates an archival HTML report from a guided workflow,
including methods, evidence, researcher decisions, deviations,
citations, and session information. See the [reproducible reporting
...
- **Core:** learning foundations (#25), research basis (#26), model
  comparison with `nomo_compare()` (#27), auditable revision lineage
  (#28), bifactor and higher-order models (#29), replication-status
  language (#30), and release certification (#37).
- **Planned:** readable invariance labels (#31), missing-data
  sensitivity (#32), score guidance (#33), insufficient-effort
  responding screens (#34), manuscript-ready tables (#35), maintenance
  (#36), and rendering reports from inside R Markdown or Quarto
  documents (#40).

Candidates for `v0.3.0` (for example IRT/DIF, Bayesian SEM, ESEM, and
